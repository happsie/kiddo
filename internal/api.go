...
func StartWebserver(addr string, pool *pgxpool.Pool) {
	...
	mux.HandleFunc("POST /api/kiddo/track-event-v1", trackHandler.CreateEvent) 
	mux.HandleFunc("GET /api/kiddo/track-event-v1", func(w http.ResponseWriter, r *http.Request) {
	})
	mux.HandleFunc("GET /api/kiddo/track-event-v1/{eventID}", func(w http.ResponseWriter, r *http.Request) {
	})
